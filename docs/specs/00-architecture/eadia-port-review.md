...
| `price_history` | **Adaptar (quase adotar)** | Já está **em cents** e **polimórfico** (`priceable_type/id`) — alinhadíssimo com a spec. | `2025_12_10_190200:25-26` já usa `unsignedInteger` cents. Bom. Ajustes menores: spec do ead2026 fala em `course_id` direto, mas o polimórfico do eadIA é **superior** — adotar polimórfico e corrigir a spec. `change_reason` como string livre (ok). | orders-payments.md (`PriceHistory`) |
